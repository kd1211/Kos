...
- **Crash resilience**: a boot-time crash now shows a real error screen
  on the LCD (instead of a silent black screen, which is what used to
  happen -- the backlight comes on before anything's ever drawn, so a
  crash before the first frame was indistinguishable from a working
  device with nothing displayed) and logs the full traceback to
  `~/.kos_crash.log` -- check that file first if something's wrong.
  A crash *after* boot in any single app (including a user-installed
  `.phoneapp`) is caught, logged the same way, and recovered from by
  falling back to Home, rather than taking the whole OS down.
